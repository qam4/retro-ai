...
- **Automated score discovery** — Run the emulator with noop/random actions, correlate RAM changes with on-screen score changes via OCR. Eliminates need for ROM disassembly.
- **Reward clipping** — Clip per-step rewards to [-1, +1] for comparable reward scales across games.
- **Frame max-pooling** — Pixel-wise max of 2 consecutive frames to handle flickering sprites.
- **Fire-on-reset** — Auto-inject fire action on episode reset for games that require it.
