...
check("action_space type is DISCRETE",
      aspace2.type == native.ActionType.DISCRETE,
      f"got {aspace2.type}")
check("action_space shape is [18]",
      aspace2.shape == [18],
      f"got {aspace2.shape}")

# R2: NOOP
result2 = env2.step([0])
check("Discrete NOOP [0] succeeds",
      not result2.truncated,
      f"truncated={result2.truncated}")

# R2: Up
result2 = env2.step([1])
check("Discrete Up [1] succeeds",
      not result2.truncated,
      f"truncated={result2.truncated}")

# R7: step_n with discrete
result2 = env2.step_n([4], 4)
check("step_n([4], 4) discrete succeeds",
      not result2.truncated,
      f"truncated={result2.truncated}")
...
try:
    ...
    check("Invalid action_mode raises InitializationError", False, "no exception raised")
except native.InitializationError as e:
    check("Invalid action_mode raises InitializationError", True)
except Exception as e:
    check("Invalid action_mode raises InitializationError", False, f"wrong exception: {type(e).__name__}: {e}")
